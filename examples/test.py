...
def get_camera_coeffs(cal_dir="camera_cal", corner_dims=(9, 6)):
    ...
    return cv2.calibrateCamera(objpoints, imgpoints, gray_imgs[0].shape[::-1], None, None)
...
def sobel(img, thresh_mag=100, thresh_dir=(0.5, 1.5)):
    ...
    return ((scaled_sobel > thresh_mag) & (dir_sobel > thresh_dir[0]) & (dir_sobel < thresh_dir[1])).astype(np.uint8)
    
    # return (scaled_sobel > thresh_mag).astype(np.uint8)

def extract_channel(image, colorspace=cv2.COLOR_RGB2HLS, thresholds=(50, 100), channel=2):
...
def sliding_window_search(wrapped_image, draw_sliding_windows=False, visualize_detected_lanes=False):
...
def draw_unwrapped(original_image, wrapped_image, unwrapped_image, left_fit, right_fit):
    ...
    cv2.putText(result, "Radius of curve is " + str(int(radius))+ "m", (100,100), 2, 1, (255,255,0), 2)
    cv2.putText(result, "Distance from center is {:2f}".format(distance)+ "m", (100,150), 2, 1, (255,255,0),2)
    return result
...
def lane_detection(img):
    ...
    #     plt.imshow(lines)
        hist = image_histogram(lines)
        w, u = perspective_transform(lines)
        w1, u1 = perspective_transform(dst)
        l, r = sliding_window_search(w)
        out = draw_unwrapped(dst, w1, u1, l, r)
        return cv2.cvtColor(out, cv2.COLOR_RGB2BGR), None
    except:
        return img, None

import jbnav

jbnav.run_experiment(lane_detection)
